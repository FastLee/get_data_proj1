...
## Test Data
x_test_fwf<-laf_open_fwf("./test/X_test.txt",rep("numeric",561), rep(16,561))
x_test<-x_test_fwf[,std_mean_features$col_no]
names(x_test)<-std_mean_features$col_name
y_test_file<-laf_open_csv("./test/subject_test.txt","numeric","Activity")
y_test<-y_test_file[,1]
sub_test_file<-laf_open_csv("./test/subject_test.txt","numeric","Subject")
sub_test<-sub_test_file[,1]
## Train Data
x_train_fwf<-laf_open_fwf("./train/x_train.txt",rep("numeric",561), rep(16,561))
x_train<-x_train_fwf[,std_mean_features$col_no]
names(x_train)<-std_mean_features$col_name
y_train_file<-laf_open_csv("./train/Y_train.txt","numeric","Activity")
y_train<-y_train_file[,1]
sub_train_file<-laf_open_csv("./train/subject_train.txt","numeric","Subject")
...
activity<-read.csv("./activity_labels.txt",sep=" ",col.names=c("Activity","Activity_Name"),colClasses=c("numeric","character"), header=FALSE)
